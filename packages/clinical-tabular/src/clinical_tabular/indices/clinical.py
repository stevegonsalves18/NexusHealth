...
This module provides validated implementations of widely-used clinical
scoring systems for kidney function, liver fibrosis, and cardiovascular
risk assessment.

Functions
---------
calculate_egfr_ckd_epi
    Estimated Glomerular Filtration Rate using the race-free 2021 CKD-EPI
    equation.  Returns CKD staging (G1–G5).

calculate_fib4_index
    Fibrosis-4 Index for non-invasive assessment of hepatic fibrosis.
    Uses age-dependent risk thresholds (<65 vs ≥65).

calculate_framingham_risk
    10-year general cardiovascular disease risk using the 2008 Framingham
    Study model with gender-specific beta coefficients.

All functions return ``None`` when inputs fail basic validation so that
callers can distinguish invalid data from a legitimate result.
"""
...
def calculate_egfr_ckd_epi(
    age: float, gender: int, creatinine: float
) -> Optional[Dict[str, Any]]:
    """Calculate eGFR using the race-free 2021 CKD-EPI equation.
...
def calculate_fib4_index(
    age: float, ast: float, alt: float, platelets: float
) -> Optional[Dict[str, Any]]:
    """Calculate the Fibrosis-4 (FIB-4) Index for liver fibrosis assessment.
...
def calculate_framingham_risk(
    ...
    """Calculate 10-year cardiovascular risk using the 2008 Framingham model.
    ...
    dict or None
        A dictionary containing:
        - ``"risk_percent"``: float (10-year risk %, rounded to 1 dp)
